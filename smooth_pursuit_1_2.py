...
class SmoothPursuit:
    def __init__(self):
        ...
        self.face_person_info = None
        self.do_smooth_pursuit_flag = False
        # grounded model
        ...
        self.loop_thread = threading.Thread(target=self._do_smooth_pursuit)
        ...
        rospy.Subscriber(
            ...
            self._collect_face_person_detect_result,
            queue_size=1,
        )
        self.pub_smooth_pursuit_result = rospy.Publisher(
            "/gaze_dm/gaze_point_target", GazePointTarget, queue_size=10
        )

    def _collect_face_person_detect_result(self, face_person_info):
        self.face_person_info = face_person_info
        self.do_smooth_pursuit_flag = True
    ...
    def _do_smooth_pursuit(self):
        while self.thread_running:
            if not self.do_smooth_pursuit_flag:
                time.sleep(0.001)
                continue

            self.do_smooth_pursuit_flag = False
            face_person_info = copy.deepcopy(self.face_person_info)
...
def main():
    ...
    smooth_pursuit = SmoothPursuit()
    try:
        rospy.spin()
    finally:
        smooth_pursuit.thread_shutdown()
