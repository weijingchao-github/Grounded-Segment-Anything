...
def check_tags_chinese(
    tags_chinese, pred_phrases, max_tokens=100, model="gpt-3.5-turbo"
):  # 将英文
...
if __name__ == "__main__":
    ...
    ram_checkpoint = os.path.join(
        os.path.dirname(__file__), args.ram_checkpoint
    )  # change the path of the model
    grounded_checkpoint = os.path.join(
        os.path.dirname(__file__), args.grounded_checkpoint
    )  # change the path of the model
    sam_checkpoint = os.path.join(os.path.dirname(__file__), args.sam_checkpoint)
    sam_hq_checkpoint = args.sam_hq_checkpoint
    use_sam_hq = args.use_sam_hq
    image_path = os.path.join(os.path.dirname(__file__), args.input_image)
    split = args.split
    ...
    # 因为tags_chinese并没有参与推理，所以要把英文推理结果映射到中文，这里好像不需要GPT，中英文tags是对应的所以可以直接匹配
