...
class SmoothPursuit:
    def __init__(self):
        ...
        self.pixel_position_x = -1
        self.pixel_position_y = -1
    ...
    def _update_target(self, gaze_point_target):
        if gaze_point_target.target_name != "":
            self.smooth_pursuit_target_name = gaze_point_target.target_name
            self.pixel_position_x = gaze_point_target.pixel_position_x
            self.pixel_position_y = gaze_point_target.pixel_position_y
            self.person_track_id = gaze_point_target.track_id
        else:
            self.smooth_pursuit_target_name = (
                ""  # 注视点在这一推理周期不再变化了,愣住了
            )
    ...
    def _do_smooth_pursuit(self, face_person_info):
        if (
            ...
            if self.smooth_pursuit_target_name == "person":
                # 先看看能不能找到脸部中心点
                find_person_face_flag = False
                for face_bbox_xyxy_and_id in face_person_info.face_bboxes_xyxy_and_ids:
                    if face_bbox_xyxy_and_id.track_id == self.person_track_id:
                        x1, y1, x2, y2 = face_bbox_xyxy_and_id.bbox_xyxy
                        self.pixel_position_x = int((x1 + x2) / 2)
                        self.pixel_position_y = int((y1 + y2) / 2)
                ...
                if not find_person_face_flag:
                    for (
                        ...
                        if person_bbox_xyxy_and_id.track_id == self.person_track_id:
                            x1, y1, x2, y2 = person_bbox_xyxy_and_id.bbox_xyxy
                            self.pixel_position_x = int((x1 + x2) / 2)
                            self.pixel_position_y = int(y1 + (y2 - y1) * 0.3)
                            find_person_bbox_face_flag = True
                            break
                if find_person_face_flag or find_person_bbox_face_flag:
                    self._pub_smooth_pursuit_result(
                        target_name="person",
                        pixel_position_x=self.pixel_position_x,
                        pixel_position_y=self.pixel_position_y,
                        depth_image=face_person_info.depth_image,
                    )
                else:  # 画面中没有这个人了
                    self._pub_smooth_pursuit_result(
                ...
                if len(pred_phrases) != 0:
                    ...
                    for index, bbox in enumerate(boxes_filt):
                        ...
                        distance = math.dist(
                            central_pixel_point,
                            (self.pixel_position_x, self.pixel_position_y),
                        )
                    ...
                    if min_pixel_distance <= self.alg_args.pixel_distance_upperbound:
                        ...
                        self.pixel_position_y = int((y1 + y2) / 2)
                        self._pub_smooth_pursuit_result(
                            target_name=self.smooth_pursuit_target_name,
                            pixel_position_x=self.pixel_position_x,
                            pixel_position_y=self.pixel_position_y,
                            depth_image=face_person_info.depth_image,
                        )
...
def main():
    rospy.init_node("/gaze_dm/smooth_pursuit")
    SmoothPursuit()
    rospy.spin()
